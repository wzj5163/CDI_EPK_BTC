# we use lower bound and upper bound as inuts
# setwd(wd)
...
bootstrap_epk = function(f, ts, currdate, out_dir, bandwidth, mon_low, mon_up){
  ...
  SPD = new.env();
  SPD$SPD = price.median^2 * exp(mean(data[,8])*tau) * d2fdX2 / mon.grid.scaled; # M=K/S_t
...
bootstrap_epk_new = function(f, ts, currdate, out_dir, bandwidth){
